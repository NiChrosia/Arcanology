...
import nichrosia.arcanology.type.nbt.impl.IntPropertyNBTEditor
import nichrosia.arcanology.type.nbt.impl.LongPropertyNBTEditor
...
abstract class MachineBlockEntity<B : Block, S : ScreenHandler, R : MachineRecipe<T, *>, T : MachineBlockEntity<B, S, R, T>>(
    ...
    open val tier = (state.block as MachineBlock<*, *, *>).tier

    open val energyStorage = BlockEntityEnergyStorage(tier)

    open val outputDirections = Direction.values()

    open val maxEnergy = tier.storage
    open val maxProgress = 1000
    open val maxSoundProgress = Registrar.sound.machinery.length

    open val progressPerTick = tier.baseProgressPerTick
    open val energyLossPerTick = tier.baseUsagePerTick
    ...
    open val delegate = KPropertyDelegate(
        ...
        MutableProperty(::maxEnergy, { it.toInt() }, { it.toLong() })
    )

    open val nbtEditors: MutableList<NBTEditor<*, *>> = mutableListOf(
        LongPropertyNBTEditor("total_energy", energyStorage::amount),
        IntPropertyNBTEditor("progress", ::progress)
    )

    open val active = BooleanProperty.of("active")
    ...
    open fun canProgress(world: World): Boolean {
        return inputSlots.any { !items[it].isEmpty } &&
            energyStorage.amount >= energyLossPerTick &&
    ...
    open fun onRecipeCompletion(world: World, pos: BlockPos, state: BlockState) {
        ...
        if (items[inputSlots[0]].isEmpty) setActiveState(world, pos, state, false)
    ...
    open fun machineTick(world: World, pos: BlockPos, state: BlockState) {
        if (canProgress(world)) {
            ...
            energyStorage.extract(energyLossPerTick, transaction)
            transaction.commit()

            markDirty()
        } else {
            if (!world.isClient) setActiveState(world, pos, state, false)
        }
    }

    /** Utility function for setting [BlockState] to active or inactive for sprite changes. */
    fun setActiveState(world: World, pos: BlockPos, currentState: BlockState, value: Boolean): BlockState {
        val state = currentState.with(active, value)
